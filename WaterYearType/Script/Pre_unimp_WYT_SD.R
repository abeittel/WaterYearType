...
quartiles <- group_by(monthlymean, COMID) %>%
  # make quartiles for each COMID for entire POR => 
  summarise(`ONEsdabove`= (mean(Estimated.Q) + sd(Estimated.Q)),
            `ONEsdbelow` = (mean(Estimated.Q) - sd(Estimated.Q)),
            `TWOsdabove`= ((mean(Estimated.Q)) + 2 * (sd(Estimated.Q))),
            `TWOsdbelow`= ((mean(Estimated.Q)) - 2 * (sd(Estimated.Q))),
            # average of all monthly means for entire POR for each COMID(gage)
            `POR_mean_annual_flow` = mean(Estimated.Q),
            `median` = median(Estimated.Q),
            # number of average monthly values used to calculate the quartiles, should be 792
            `Count_monthlyavg`= n())
View(quartiles)

COM1 <- left_join(monthlymean, quartiles, by = "COMID") 
COM1 <- filter(COM1, COMID == 342587)
View(COM1)

plot(COM1$Water_Month, COM1$Estimated.Q, lty = 2, xlab = "Water Month", ylab = "Monthly Mean CFS", main = "POR Flow Distribution for COMID 342587 - HIGHLAND C BL SPICER MEADOWS RES")
?plot
...
WYT_all <- mutate(all, WYT = ifelse(Mean_Annual_Flow >= ONEsdbelow & Mean_Annual_Flow <= ONEsdabove, "Wet",
                             ifelse(Mean_Annual_Flow >= TWOsdbelow & Mean_Annual_Flow <= TWOsdabove 
                                    & Mean_Annual_Flow <= ONEsdbelow & Mean_Annual_Flow >= ONEsdabove, "Above Moderate",
                             ifelse(Mean_Annual_Flow < TWOsdbelow & Mean_Annual_Flow > TWOsdabove, "Dry", NA))))
...
WYT_final <- select(WYT_USGS_all, USGS_GAGE, COMID, STATION_NA, Water_Year, ONEsdabove, ONEsdbelow, TWOsdabove, TWOsdbelow, Mean_Annual_Flow, WYT, POR_mean_annual_flow, Count_water_months, Count_monthlyavg)
View(WYT_final)
#b) export dataframe as csv
write.csv(WYT_final, "X:/environmental_flows/WaterYearType/Results/SD_Modeled.Predicted.Unimpaired.15+.WYT_09.27.2018.csv")
