...
#2.1) Calculate quartiles for each COMID => quartiles are based on the entire POR for for 1) Oct-Dec and then 2) for Jan-Mar
#a) Omit all 0 Estimated.Q values
monthlymean[monthlymean==0] <- NA
monthlymean <- na.omit(monthlymean)
#b) calculate quartiles for Oct-Mar 
monthlymean <- filter(monthlymean, Water_Month < 7 &  Water_Month > 0)
View(monthlymean)

OMquartiles <- group_by(monthlymean, COMID) %>%
  # make quartiles for each COMID for entire POR => Q1=25%, Q2=50%, Q3=75%
  summarise(`Q1`=quantile(Estimated.Q, probs=0.25),
            `Q2`=quantile(Estimated.Q, probs=0.5),
            `Q3`=quantile(Estimated.Q, probs=0.75),
...
#b) calculate quartiles for Oct-Dec 
monthlymean <- filter(monthlymean, Water_Month < 4 &  Water_Month > 0)
View(monthlymean)

ODquartiles <- group_by(monthlymean, COMID) %>%
  # make quartiles for each COMID for entire POR => Q1=25%, Q2=50%, Q3=75%
  summarise(`Q1`=quantile(Estimated.Q, probs=0.25),
            `Q2`=quantile(Estimated.Q, probs=0.5),
            `Q3`=quantile(Estimated.Q, probs=0.75),
...
#b) calculate quartiles for Jan-Mar 
monthlymean <- filter(monthlymean, Water_Month < 7 &  Water_Month > 3)
View(monthlymean)

JMquartiles <- group_by(monthlymean, COMID) %>%
  # make quartiles for each COMID for entire POR => Q1=25%, Q2=50%, Q3=75%
  summarise(`Q1`=quantile(Estimated.Q, probs=0.25),
            `Q2`=quantile(Estimated.Q, probs=0.5),
            `Q3`=quantile(Estimated.Q, probs=0.75),
...
all <- left_join(annualmean, ODquartiles, by = "COMID")
View(all)
...
View(WYT_final)
#b.1) export dataframe as csv for Oct-Mar
write.csv(WYT_final, "X:/environmental_flows/WaterYearType/Results/Oct-Mar_Modeled.Predicted.Unimpaired.15+.WYT_09.26.2018.csv")
write.csv(WYT_final, "X:/environmental_flows/WaterYearType/Results/NA_Oct-Mar_Modeled.Predicted.Unimpaired.15+.WYT_09.26.2018.csv")

#b.2) export dataframe as csv for Oct-Dec
write.csv(WYT_final, "X:/environmental_flows/WaterYearType/Results/Oct-Dec_Modeled.Predicted.Unimpaired.15+.WYT_09.26.2018.csv")

#b.3) export dataframe as csv for Jan-Mar
write.csv(WYT_final, "X:/environmental_flows/WaterYearType/Results/Jan-Mar_Modeled.Predicted.Unimpaired.15+.WYT_09.26.2018.csv")
